...
class DQNAgent:
    ...
    def update_model(self):
        ...
        print(f"[DQN] Batch loss: {loss.item():.4f}  |  "
              f"Q_mean: {q_values.mean().item():.3f}  |  Q_max: {q_values.max().item():.3f}")
        print(f"[DQN] Replay Buffer Size: {len(self.memory)}")
...
if __name__ == "__main__":
    ...
    agent = DQNAgent()
    agent.train(num_episodes=num_training_episodes, target_update=10)
    agent.save_agent(agent.best_model_path)
    agent.test(num_test_episodes=100)